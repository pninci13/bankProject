...
public class ManagerWithdrawController implements Initializable {
    ...
    public void onManagerWithdrawButtonClick(ActionEvent event) {
        ...
        if (value > 0) {
            if (value >= 110000) {
                    ...
                    for (int j = 0; j < client.getAccounts().size(); j++) {
                        ...
                        if (String.valueOf(account.getAccountNumber()).equals(number[1])) {
                            if(account.withdraw(value)){
                                ...
                                Alert message = new Alert(Alert.AlertType.INFORMATION);
                                message.setContentText("Balance is not enough! ");
                                message.setTitle("Unsuccessful Withdraw");
                                message.setGraphic(imageView);
                                message.showAndWait();
                                amountValue.setText("");
                ...
                Alert message = new Alert(Alert.AlertType.INFORMATION);
                message.setContentText("Amount is lower than your range! ");
                message.setTitle("Unsuccessful Withdraw");
                message.setGraphic(imageView);
                message.showAndWait();
                amountValue.setText("");
